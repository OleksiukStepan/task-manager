...
class CustomFilterTests(TestCase):
    def test_priority_class_filter(self):
        template = Template("{% load custom_filters %}{{ 1|priority_class }}")
        rendered = template.render(Context())
        self.assertEqual(rendered, "text-danger")

        template = Template("{% load custom_filters %}{{ 4|priority_class }}")
        rendered = template.render(Context())
    ...
    def test_load_tags_inclusion_tag(self):
        ...
        template = Template("{% load custom_filters %}{% load_tags %}")
        rendered = template.render(Context())
